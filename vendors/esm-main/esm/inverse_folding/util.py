...
except ImportError:  # pragma: no cover - optional dependency for structure IO only
    biotite = None
    pdbx = None
    pdb = None
    get_residues = None
...
def require_biotite() -> None:
    if biotite is None:
        raise ImportError(
            "biotite is only required for the legacy structure-loading helpers in esm.inverse_folding.util. "
            "The MarsStack gemmi-based ESM-IF runner can operate without biotite."
        )
